...
class ChatAgentDependencies:
    ...
    personality_switch_enabled: bool = True
...
async def instructions_with_personality_switch(ctx: pydantic_ai.RunContext[ChatAgentDependencies]) -> str:
    ...
    return AGENT_PROMPT.format(
        ...
        if ctx.deps.personality_switch_enabled
        else NO_PERSONALITY_SWITCH_INSTRUCTIONS,
...
async def validate_agent_response(
    ...
    if ctx.deps.personality_switch_enabled is False:
        if data.response_type == "SwitchPersonalityResponse":
            raise PersonalitySwitchDisabledError()
