...
from areyouok_telegram.llms.utils import get_unsupported_media_from_messages
...
class TestGetUnsupportedMediaFromMessages:
    """Test the get_unsupported_media_from_messages function."""

    @pytest.mark.asyncio
    async def test_no_media(self):
        """Test with messages that have no media."""
        mock_message = MagicMock(spec=telegram.Message)
        ...
        with patch("areyouok_telegram.llms.utils.MediaFiles.get_by_message_id", new=AsyncMock(return_value=[])):
            mock_db_conn = AsyncMock()
            result = await get_unsupported_media_from_messages(mock_db_conn, [mock_message])

        assert result == []

    @pytest.mark.asyncio
    async def test_supported_media_types(self):
        """Test that supported media types are not returned."""
        mock_message = MagicMock(spec=telegram.Message)
        mock_message.from_user = MagicMock()
        mock_message.date = datetime.now(UTC)
        mock_message.chat.id = 123
        ...
        # Create supported media mocks
        mock_image = MagicMock(spec=MediaFiles)
        mock_image.mime_type = "image/jpeg"

        mock_pdf = MagicMock(spec=MediaFiles)
        mock_pdf.mime_type = "application/pdf"

        mock_text = MagicMock(spec=MediaFiles)
        mock_text.mime_type = "text/plain"

        mock_audio = MagicMock(spec=MediaFiles)
        mock_audio.mime_type = "audio/mp3"

        with patch(
            "areyouok_telegram.llms.utils.MediaFiles.get_by_message_id",
            new=AsyncMock(return_value=[mock_image, mock_pdf, mock_text, mock_audio]),
        ):
            mock_db_conn = AsyncMock()
            result = await get_unsupported_media_from_messages(mock_db_conn, [mock_message])
    ...
    async def test_unsupported_media_types(self):
        """Test that unsupported media types are returned."""
        mock_message = MagicMock(spec=telegram.Message)
        mock_message.from_user = MagicMock()
        mock_message.date = datetime.now(UTC)
        mock_message.chat.id = 123
        ...
        # Create unsupported media mocks
        mock_video = MagicMock(spec=MediaFiles)
        mock_video.mime_type = "video/mp4"

        mock_zip = MagicMock(spec=MediaFiles)
        mock_zip.mime_type = "application/zip"

        with patch(
            "areyouok_telegram.llms.utils.MediaFiles.get_by_message_id",
            new=AsyncMock(return_value=[mock_video, mock_zip]),
        ):
            mock_db_conn = AsyncMock()
            result = await get_unsupported_media_from_messages(mock_db_conn, [mock_message])

        assert set(result) == {"video/mp4", "application/zip"}

    @pytest.mark.asyncio
    async def test_skip_messages_before_timestamp(self):
        """Test that messages before timestamp are skipped."""
        timestamp = datetime.now(UTC)

        mock_old_message = MagicMock(spec=telegram.Message)
        mock_old_message.from_user = MagicMock()
        mock_old_message.date = timestamp - timedelta(hours=1)
        mock_old_message.chat.id = 123
        mock_old_message.message_id = 1

        mock_new_message = MagicMock(spec=telegram.Message)
        mock_new_message.from_user = MagicMock()
        mock_new_message.date = timestamp + timedelta(hours=1)
        mock_new_message.chat.id = 123
        mock_new_message.message_id = 2

        mock_video = MagicMock(spec=MediaFiles)
        mock_video.mime_type = "video/mp4"

        with patch(
            "areyouok_telegram.llms.utils.MediaFiles.get_by_message_id", new=AsyncMock(return_value=[mock_video])
        ) as mock_get_media:
            mock_db_conn = AsyncMock()
            result = await get_unsupported_media_from_messages(
                mock_db_conn, [mock_old_message, mock_new_message], since_timestamp=timestamp
            )

        # Should only check the new message
        assert mock_get_media.call_count == 1
        call_args = mock_get_media.call_args_list[0]
        assert call_args.kwargs["message_id"] == "2"

        assert result == ["video/mp4"]

    @pytest.mark.asyncio
    async def test_skip_bot_messages(self):
        """Test that bot messages (no from_user) are skipped."""
        mock_bot_message = MagicMock(spec=telegram.Message)
        mock_bot_message.from_user = None  # Bot message
        mock_bot_message.date = datetime.now(UTC)
        mock_bot_message.chat.id = 123
        mock_bot_message.message_id = 1

        with patch("areyouok_telegram.llms.utils.MediaFiles.get_by_message_id", new=AsyncMock()) as mock_get_media:
            mock_db_conn = AsyncMock()
            result = await get_unsupported_media_from_messages(mock_db_conn, [mock_bot_message])

        # Should not check media for bot messages
