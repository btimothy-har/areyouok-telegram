...
class TestTranscribeVoiceDataSync:
    ...
    def test_transcribe_short_audio(self):
        ...
        with (
            patch("areyouok_telegram.handlers.media.AudioSegment.from_ogg", return_value=mock_audio_segment),
            patch("areyouok_telegram.handlers.media.openai.OpenAI", return_value=mock_client),
            patch("areyouok_telegram.handlers.media.OPENAI_API_KEY", "test-key"),
        ):
    ...
    def test_transcribe_voice_data_sync_exception(self):
        ...
        with patch(
            "areyouok_telegram.handlers.media.AudioSegment.from_ogg",
...
class TestDownloadFile:
    ...
    async def test_download_file_non_voice(self, mock_db_session):
        ...
        with (
            patch("areyouok_telegram.handlers.media.MediaFiles.create_file", new=AsyncMock()) as mock_create_file,
            patch("areyouok_telegram.handlers.media.logfire.span"),
            patch("areyouok_telegram.handlers.media.logfire.info"),
        ):
    ...
    async def test_download_voice_file_with_transcription(self, mock_db_session):
        ...
        with (
            patch("areyouok_telegram.handlers.media.MediaFiles.create_file", new=AsyncMock()) as mock_create_file,
            patch(
                "areyouok_telegram.handlers.media.asyncio.to_thread",
                new=AsyncMock(return_value=mock_transcriptions),
            ),
            patch("areyouok_telegram.handlers.media.LLMUsage.track_generic_usage", new=AsyncMock()) as mock_track_usage,
            patch("areyouok_telegram.handlers.media.logfire.span"),
            patch("areyouok_telegram.handlers.media.logfire.info") as mock_log_info,
        ):
            await _download_file(mock_db_session, chat_encryption_key, message=mock_message, file=mock_file)
    ...
    async def test_download_voice_file_transcription_error(self, mock_db_session):
        ...
        with (
            ...
            patch("areyouok_telegram.handlers.media.logfire.span"),
            patch("areyouok_telegram.handlers.media.logfire.exception") as mock_log_exception,
        ):
            await _download_file(mock_db_session, chat_encryption_key, message=mock_message, file=mock_file)
    ...
    async def test_download_file_general_exception(self, mock_db_session):
        ...
        with patch("areyouok_telegram.handlers.media.logfire.exception") as mock_log_exception:
            await _download_file(mock_db_session, chat_encryption_key, message=mock_message, file=mock_file)
...
class TestExtractMediaFromTelegramMessage:
    ...
    async def test_extract_no_media(self, mock_db_session):
        ...
        with (
            patch("areyouok_telegram.handlers.media._download_file", new=AsyncMock()) as mock_download,
            patch("areyouok_telegram.handlers.media.logfire.info") as mock_log_info,
        ):
    ...
    async def test_extract_media_with_download_exceptions(self, mock_db_session):
        ...
        with (
            patch("areyouok_telegram.handlers.media._download_file", mock_download),
            patch("areyouok_telegram.handlers.media.logfire.info") as mock_log_info,
        ):
            result = await extract_media_from_telegram_message(
                mock_db_session, "test_encryption_key", message=mock_message
