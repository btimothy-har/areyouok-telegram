...
@pytest.mark.asyncio
async def test_track_dspy_usage_error_handling(mock_async_database_session):
    """Test that dspy tracking errors don't break application flow."""
    # Mock the database to throw an error
    mock_async_database_session.execute.side_effect = Exception("Database error")
    ...
    result = await LLMUsage.track_dspy_usage(
        session=mock_async_database_session,
        chat_id="test_chat_dspy_error",
        session_id="test_session_dspy_error",
        usage_type=mock_usage_type,
        data=usage_data,
    )
